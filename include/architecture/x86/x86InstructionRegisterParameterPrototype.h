    ...
    specify(const X86InstructionRegisterParameterGroup& registerGroup) const {
        return *(
            std::find_if(std::cbegin(registerGroup), std::cend(registerGroup),
                         [](const X86InstructionRegisterParameter&
                                currentRegisterParameter) {
...
struct X86InstructionSingleRegisterParameterPrototypeSpecification {
  public:
    ...
    X86InstructionSingleRegisterParameterPrototypeSpecification& operator=(
        const X86InstructionSingleRegisterParameterPrototypeSpecification&);
    X86InstructionSingleRegisterParameterPrototypeSpecification&
    operator=(X86InstructionSingleRegisterParameterPrototypeSpecification&&);

  private:
    std::string _registerName;
};

using X86InstructionRegisterParameterPrototype_t =
    std::variant<X86InstructionSingleRegisterParameterPrototypeSpecification,
...
const auto x86InstructionRegisterParameterPrototypeGetSize =
    [](const auto& registerPrototype) -> ParameterSize {
    return registerPrototype.size();
};

auto x86InstructionRegisterParameterPrototypeSpecify = make_visitor(
    []<RegisterSize T>(
        const X86InstructionRegisterParameterPrototypeSpecification<T>& ref,
        const X86InstructionRegisterParameterGroup& registerGroup)
        -> std::shared_ptr<InstructionParameter> {
        return std::make_shared<X86InstructionRegisterParameter>(
            ref.specify(registerGroup));
    },
    [](const X86InstructionSingleRegisterParameterPrototypeSpecification& ref,
       const X86InstructionRegisterParameterGroup& registerGroup)
        -> std::shared_ptr<InstructionParameter> {
        return std::make_shared<X86InstructionRegisterParameter>(
            ref.specify(registerGroup));
    },
    [](const auto&, const X86InstructionRegisterParameterGroup&)
