...
template <typename T> class BidirectionalIterator {
  public:
    ...
    BidirectionalIterator<T>(const pointer start, const int64_t length)
        : _dataPointer(start), _dataStart(start), _dataEnd(start + (length)) {}
    BidirectionalIterator<T>(const pointer position, const pointer start,
                             const int64_t end)
        : _dataPointer(position), _dataStart(start), _dataEnd(end) {}

    bool canMove(int64_t amount) {
        if (amount == 0) {
            isValid();
        } else {
            return isValid() && ((_dataPointer + amount) >= _dataStart) &&
                   ((_dataPointer + amount) < _dataEnd);
        }
    }
    ...
    BidirectionalIterator<T> operator++() {
        ...
        //        throw std::range_error("Attempt to move BidirectionalIterator
        //        beyond the bounds of the data that it iterates over");
        //}
        ++_dataPointer;
        return *this;
    }
    ...
    reference operator*() const {
        if (_dataPointer >= _dataEnd || _dataPointer < _dataStart) {
            throw std::range_error(
                "Attempt to access BidirectionalIterator while it points "
                "outside the bounds of the data that it iterates over");
        }
        return *_dataPointer;
    }
