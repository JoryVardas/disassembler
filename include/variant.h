...
template <typename... Ty> struct _variant_join_helper;
template <typename... A, typename... B>
struct _variant_join_helper<TypeCollection<A...>, TypeCollection<B...>> {
    using _type = TypeCollection<A..., B...>;
};

template <typename... Ty> struct _variant_join;
template <typename NonVariantType> struct _variant_join<NonVariantType> {
...
struct _variant_join<FirstVariant, RemainingVariants...> {
    ...
    using _remainingVariantType =
        typename _variant_join<RemainingVariants...>::_type;
    using _type = typename _variant_join_helper<_firstVariantType,
                                                _remainingVariantType>::_type;
};

template <typename... Variants>
using variant_join = typename change_container<
    std::variant<>, typename _variant_join<Variants...>::_type>::_type;

template <typename... Ty> struct _holds_any_alternative_helper;
template <typename Variant, typename... VariantTypes, typename... Alternatives>
struct _holds_any_alternative_helper<Variant, std::variant<VariantTypes...>,
...
bool holds_any_alternative(Variant& var) {
    return _holds_any_alternative_helper<
...
// Kline
template <typename... FuncTypes> struct _make_visitor_helper;

template <typename FuncType> struct _make_visitor_helper<FuncType> : FuncType {
...
struct _make_visitor_helper<FuncType, RestFuncTypes...>
    : FuncType, _make_visitor_helper<RestFuncTypes...> {
    _make_visitor_helper(FuncType func, RestFuncTypes... rest)
        : FuncType(func), _make_visitor_helper<RestFuncTypes...>(rest...) {}
